(function ($) {
    $.createMenu = function (p) {
        ...
        var MenuMgr = {
            ...
            init: function () {
                MenuMgr.LoadDefaultMenuFromSetting();
                ...
                $('.sfUploadedFilesExtLink').on("click", ".deleteIcon", function () {
                ...
                $('#menuList').on('click', '.sfMenuChecked', function (index, value) {
            ...
            LoadDefaultMenuFromSetting: function () {
                ...
                this.config.url = MenuMgr.config.baseURL + MenuMgr.config.method;
            ...
            BindMenuList: function (data) {
                ...
                $.each(LstMenu, function (index, item) {
                    if (item != "") {
                        ...
                        var checked = '<span class="sfMenuChecked icon-unchecked"></span>';
                        if (item.MenuID == activeMenuID) {
                            checked = '<span class="sfMenuChecked icon-checked"></span>';
                        }
                        if (!item.IsDefault) {
                            menulist += '<li id=' + item.MenuID + '>' + checked + '<span class="sfMenuname">' + item.MenuName + '</span><span class="sfMenuOption"> <i class="MenuTypeEdit icon-edit"></i>';
                            menulist += '<i class="delete icon-delete" id=' + delid + '></i></span>';
                        }
                        else {
                            menulist += '<li id=' + item.MenuID + '>' + checked + '<span class="sfMenuname">' + item.MenuName + '</span><span class="sfMenuOption"> <i class="MenuTypeEdit icon-edit"></i></span>';
            ...
            UpdateSelectedMenu: function (menuID) {
                MenuMgr.config.method = "SaveSageMenu";
                MenuMgr.config.data = JSON2.stringify({
                    UserModuleID: MenuMgr.config.UserModuleID,
                    PortalID: MenuMgr.config.PortalID,
                    SettingKey: 'MenuID',
                    SettingValue: menuID,
                ...
                switch (MenuMgr.config.ajaxCallMode) {
                    ...
                    case 13:
                        ...
                        $.each(menu, function (key, value) {
                            activeMenuID = value.SelectedMenu;
                        });
                        MenuMgr.LoadMenu();
                        break;
                    case 14:
                        $('#menuList ul li').find('.sfMenuChecked.icon-checked').removeClass('icon-checked').addClass('icon-unchecked');
                        $('#menuList ul li#' + activeMenuID).find('.sfMenuChecked').addClass('icon-checked').removeClass('icon-unchecked');
                        $('#menuList').find('#' + activeMenuID).find('.sfMenuname').trigger('click');
                        break;
                }
            },
            ...
            IconUploaderExtLink: function () {
                ...
                var upload = new AjaxUpload($('#fupIconExtLink'), {
                    ...
                    onComplete: function (file, response) {
                        ...
                        var html = '<div><img class="sfIcon" title="' + Eximage + '" src="' + filePath + '" /><span class="deleteIcon icon-close"><i class="delete"></i></span></div>';
                        $('div.sfUploadedFilesExtLink').html(html);
                    }
                });
            },
