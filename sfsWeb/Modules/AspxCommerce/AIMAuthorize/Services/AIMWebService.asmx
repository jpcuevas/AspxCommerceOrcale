<%@ WebService Language="C#" Class="AIMWebService" %>
...
public class AIMWebService  : System.Web.Services.WebService {
        ...
        try
        {
            ...
            if (objRetVals[0].Trim(char.Parse("|")) == "1")
            {
               ...
               // StoreSettingConfig ssc = new StoreSettingConfig();
                ...
                if (HttpContext.Current.Session["IsFreeShipping"] != null)
                {
                    acc.ClearSessionVariable("IsFreeShipping");

                }
                if (HttpContext.Current.Session["DiscountAmount"] != null)
                {
                    acc.ClearSessionVariable("DiscountAmount");

                }
                if (HttpContext.Current.Session["CouponCode"] != null)
                {
                    acc.ClearSessionVariable("CouponCode");
